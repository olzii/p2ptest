...
$(document).ready(function(){
            ...
            function draw_users() {
				...
				for(i=0;i<users.length;i++){
                    var ss ='';
                    if(users[i].status==3) ss=' (typing...)'
					listHtml += '<li>'+users[i].name+ss+'</li>';
				}
				$('#userList').html(listHtml);
            }
            function set_user_status(name, status) {
				for(i=0;i<users.length;i++){
                    if(users[i].name==name) users[i].status = status;
				}
				draw_users()
            }
			socket.on('users',function(data){
                ...
                for(i=0;i<data.length;i++) {
                    users.push({
                        name : data[i],
                        status : 2 // 0-off, 1-idle, 2-active, 3-typing
                    });
                }
				draw_users();
			});
			socket.on('message',function(data){
                set_user_status(data.name, 2); // 0-off, 1-idle, 2-active, 3-typing
				$('#chatScroll>ul').prepend('<li><b>'+data.name+': </b>'+data.msg+'</li>');
			});
            $msg.keypress(function(){
                socket.emit('typing', profile.userid);
                //console.log(profile.userid+' is typing...');
            })
            $msg.keyup(function(e){
                if(e.keyCode == 8 || e.keyCode == 46) {
                    var msg = $('#message').val();
                    if(msg.trim().length==0) socket.emit('typing-cancel', profile.userid);
                }
            })
            socket.on('typing', function(data){
                //console.log('socket, '+data+' is typing...');
                set_user_status(data, 3); // 0-off, 1-idle, 2-active, 3-typing
            });
            socket.on('typing-cancel', function(data){
                set_user_status(data, 2); // 0-off, 1-idle, 2-active, 3-typing
            });
